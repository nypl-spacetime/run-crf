...
const findFields = (row) => {
  var fieldPositions = R.toPairs(row.fields)
    .map((kv) => {
      if (kv[1]) {
        const from = row.input.indexOf(kv[1])
        return {
          field: kv[0],
          range: [from, from + kv[1].length - 1]
        }
      } else {
        return null
      }
  ...
  var inputPos = 0
  var lines = fieldPositions
    .map((field, index) => {
      const last = index === fieldPositions.length - 1
      var lines = []
      if (field.range[0] > inputPos) {
        lines.push({
          text: row.input.substring(inputPos, field.range[0]),
          field: OTHER_FIELD
        })
      }

      lines.push({
        text: row.input.substring(field.range[0], field.range[1] + 1),
        field: field.field
      })
      inputPos = field.range[1] + 1

      if (last && inputPos < row.input.length) {
        lines.push({
          text: row.input.substring(inputPos, row.input.length),
          field: OTHER_FIELD
        })
      }
